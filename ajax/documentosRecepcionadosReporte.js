                ...
                if (Array.isArray(data) && data.length > 0) {
                    let rows = data.map(documento => `
                        <tr>
                            ...
                            <td>
                                <span class="estado ${documento['estado documento'] === 'a' ? 'follow' : documento['estado documento'] === 'i' ? 'finished' : 'new'}">
                                    ${documento['estado documento'] === 'a' ? 'En seguimiento' : documento['estado documento'] === 'i' ? 'Seguimiento finalizado' : 'Pendiente de envío'}
                                </span>
                            </td>
                        </tr>
                    `).join('');
